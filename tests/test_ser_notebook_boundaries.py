...
"""
    ...
    def test_decoder_notebook_is_cache_only_and_formal_run_is_disabled(self):
        ...
        self.assertIn("TrainingMonitoringConfig(max_epoch_samples=2000, sampling_seed=0)", code_source)
        self.assertIn("monitoring_config=TRAINING_MONITORING_CONFIG", code_source)
        ...
        self.assertIn("monitoring_config=MSP_MONITORING_CONFIG", code_source)
        self.assertIn("run_msp_loss_comparison", code_source)
        self.assertIn("SER_HCUDB1_MANIFEST", source_text(working_cells["study-artifact-paths"]))
        self.assertIn("SER_HCUDB1_CACHE", source_text(working_cells["study-artifact-paths"]))
